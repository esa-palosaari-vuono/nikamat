...
enum ExerciseLibrary {
    ...
    static let all: [Exercise] = [
        ...
        // Isometrics: the hand resists and the head does not move. These are
        // the strength component the break can offer without equipment, and
        // the gaze never leaves the screen. The figure barely moves by design;
        // the phase label carries the instruction.

        Exercise(
            id: "neck-iso-flexion",
            name: "Niskan isometria eteen",
            region: .neck,
            posture: .seated,
            tiers: [.micro, .long],
            orientation: .front,
            highlight: .neck,
            prop: nil,
            cue: "Kämmen otsalle ja paina otsaa kättä vasten",
            note: "Pää ei liiku, käsi vastustaa. Noin puolet täydestä voimasta riittää.",
            sides: nil,
            pattern: .reps(count: 5, hold: 7, release: 3),
            keyframes: [
                Pose(right: ArmPose(anchor: .headPoint(CGPoint(x: 0.02, y: -0.24)), elbowOut: 0.55)),
                Pose(
                    headNod: 2,
                    right: ArmPose(anchor: .headPoint(CGPoint(x: 0.02, y: -0.24)), elbowOut: 0.55)
                )
            ]
        ),
        ...
        Exercise(
            ...
            highlight: .neck,
            prop: nil,
            cue: "Sormet ristiin takaraivolle ja paina päätä taaksepäin käsiä vasten",
            note: "Leuka pysyy sisään vedettynä eikä pää liiku. Noin puolet täydestä voimasta.",
            sides: nil,
            pattern: .reps(count: 5, hold: 7, release: 3),
            keyframes: [
                {
                ...
                {
                    let a = both(ArmPose(anchor: .headBack, elbowOut: 0.30))
                    return Pose(headNod: -2, scapulaSqueeze: 0.15, left: a.left, right: a.right)
                }()
            ]
        ),
        ...
        Exercise(
            ...
            highlight: .neck,
            prop: nil,
            cue: "Kämmen ohimolle ja paina päätä kättä vasten",
            note: "Pää pysyy suorassa ja vastakkainen hartia alhaalla. Noin puolet täydestä voimasta.",
            sides: ["oikealle", "vasemmalle"],
            pattern: .reps(count: 3, hold: 7, release: 3),
            keyframes: [
                Pose(right: ArmPose(anchor: .headPoint(CGPoint(x: 0.38, y: -0.02)))),
                Pose(
                    headTilt: 2,
                    left: ArmPose(shoulderLift: -0.15),
                    right: ArmPose(anchor: .headPoint(CGPoint(x: 0.38, y: -0.02)))
                )
            ]
        ),
